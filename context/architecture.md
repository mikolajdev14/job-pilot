...
│   └── api/
│       ├── auth/
│       │   └── refresh/route.ts            → Refresh InsForge auth session
│       ├── agent/
...
├── actions/
│   ├── auth.ts                             → OAuth and sign-out Server Actions
│   ├── profile.ts                          → Profile save + update
│   └── jobs.ts                            → Job status updates
├── components/
│   ├── ui/                                → shadcn/ui components only
│   ├── auth/
│   │   └── LoginForm.tsx                  → OAuth provider buttons
│   ├── layout/
│   │   ├── Navbar.tsx
│   │   └── Footer.tsx
│   ├── homepage/
...
│   ├── auth-constants.ts                  → Shared auth cookie names
...
├── proxy.ts                                → Refresh and protect authenticated routes
└── types/
...
- Public routes: /, /login
- Proxy in proxy.ts refreshes the session before protected routes
...
// Browser-side — used in client components for authenticated SDK reads
...
// Server-side — used in API routes, Server Components, and agents
...
- All InsForge server-side reads use `createInsforgeServer()` and auth mutations use `createAuthActions()` — never the browser client.
